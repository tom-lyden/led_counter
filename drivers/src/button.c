...
static void start_debounce_timer(void* context);
...
bool_t Button_Init(button_t* button, const button_cfg_t* cfg)
{
    ...
    button->gpio_state = GPIO_Read(cfg->gpio_port, cfg->gpio_pin);
...
void Button_Update(button_t* button)
{
    ...
    if (gpio_state == button->gpio_state)
        return;
    
    button->gpio_state = gpio_state;
